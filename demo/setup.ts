...
async function setup() {
  ...
  console.log(`🚀 Setting up x402 Facilitator Demo (${network.name})`);
  console.log('===================================================');
  console.log(`📂 Data directory: ${DATA_DIR}`);
  console.log(`🌐 Network: ${network.name} (chainId: ${network.chainId})`);
  console.log(`🔗 RPC: ${network.rpcUrl}`);
  console.log(`🪙 SBC Token: ${network.sbcAddress} (${network.sbcDecimals} decimals)`);
  ...
  if (needsFunding) {
    ...
    if (network.cliName === 'base-sepolia') {
  ...
  // Map cliName to env var prefix (testnets get their own prefix)
  ...
  const envPrefix = envPrefixMap[network.cliName] || 'BASE';

  const envContent = `
# Generated by demo/setup.ts (network: ${network.cliName})
...
`.trim();
  ...
  console.log(`   2. Run the demo client: npm run demo -- --network ${network.cliName}`);
}
