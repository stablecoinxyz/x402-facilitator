...
`Permit(owner, spender, value, nonce, deadline)` has no recipient field. The service verifies only that the permit authorizes its facilitator as spender ([`src/routes/verify.ts`](../src/routes/verify.ts#L351)), then takes the transfer recipient directly from `paymentRequirements.payTo` ([`src/routes/settle.ts`](../src/routes/settle.ts#L417)) and transfers the full permit value to it ([`src/routes/settle.ts`](../src/routes/settle.ts#L784)). The apparent recipient check in verify compares `payTo` with itself ([`src/routes/verify.ts`](../src/routes/verify.ts#L452)).
...
The signed Solana message includes a nonce, but that nonce is not consumed by the SPL Token program. Replay state is only the in-memory `NonceTracker` ([`src/protection/nonce-tracker.ts`](../src/protection/nonce-tracker.ts#L13)). After `sendRawTransaction` succeeds but confirmation errors, settlement returns `settlement_pending` ([`src/solana/settle.ts`](../src/solana/settle.ts#L102)); the caller records a nonce only for `success` ([`src/routes/settle.ts`](../src/routes/settle.ts#L341)). A retry can therefore broadcast the same delegated transfer again. Restarting or running another instance also empties the replay record.
...
The code permits `value >= paymentRequirements.amount` ([`src/routes/settle.ts`](../src/routes/settle.ts#L550)) but passes `value`, not the required amount, to `transferFrom` ([`src/routes/settle.ts`](../src/routes/settle.ts#L795)). A user who signed a permit with headroom can be overcharged.
...
For Radius, `eth_estimateGas` is skipped ([`src/routes/settle.ts`](../src/routes/settle.ts#L687)) and a well-formed but invalid ECDSA signature proceeds to a real `permit` transaction. Since settlement is permissionless, attackers can repeatedly consume facilitator gas with their own invalid payloads.
...
The per-wallet settlement queue has no maximum length, timeout, cancellation, or admission control ([`src/lib/settlement-queue.ts`](../src/lib/settlement-queue.ts#L20)). The rate limiter is an in-memory IP map ([`src/protection/rate-limiter.ts`](../src/protection/rate-limiter.ts#L15)); it does not coordinate instances and, without Express proxy trust configuration, may see the reverse proxy rather than the client IP. A distributed requester can retain unbounded requests while an RPC call or receipt wait stalls.
...
The Casper verification and settlement catches return `error.message` directly ([`src/casper/verify.ts`](../src/casper/verify.ts#L126), [`src/casper/settle.ts`](../src/casper/settle.ts#L70)). The upstream response's error text is included in that message by the client. This can disclose upstream implementation, request, or provider details.
